...
class AuthService {
  ...
  def generateJwt(email: String): IO[Exception, String] = {
    JwtHandling.generateJwt(email)
  }

  def verifyJwt(jwtToken: String): IO[Exception, String] = {
    JwtHandling.verifyJwt(jwtToken)
  }
  ...
  private object JwtHandling {
    ...
    private final val YouShouldNotKeepSecretsHardcoded = "#>!IEd!G-L70@OTr$t8E[4.#[A;zo2@{"

    private final val algorithm: Algorithm = Algorithm.HMAC256(YouShouldNotKeepSecretsHardcoded)
...
object AuthService:
  val live: ZLayer[Any, Nothing, AuthService] = ZLayer.succeed(AuthService())
