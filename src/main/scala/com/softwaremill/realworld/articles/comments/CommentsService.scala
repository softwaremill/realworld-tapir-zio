...
class CommentsService(
  ...
  def addComment(slug: ArticleSlug, userId: Int, comment: String): Task[Comment] = for {
    ...
    _ <- ZIO.logInfo(SuccessfullyAddedCommentMessage(comment))
  } yield comment
  ...
  def deleteComment(slug: ArticleSlug, userId: Int, commentId: CommentId): Task[Unit] = for {
    ...
    _ <- ZIO.logInfo(SuccessfullyDeletedCommentMessage(commentId))
  } yield ()
...
object CommentsService:
  ...
  private val SuccessfullyAddedCommentMessage: Comment => String = (comment: Comment) => s"Successfully created comment $comment"
  private val SuccessfullyDeletedCommentMessage: CommentId => String = (commentId: CommentId) =>
    s"Successfully deleted comment with id=${commentId.value}"
